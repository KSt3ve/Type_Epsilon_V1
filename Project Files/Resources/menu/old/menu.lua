...
function menu:FlyIn(dir)
    self.alpha = 1
    if dir == 'left' then
        self.x = screen.width * 0.2 - screen.width
    elseif dir == 'right' then
        self.x = screen.width * 0.5 + screen.width
    end
    task.Clear(self)
    task.New(self, function()
        task.MoveTo(screen.width * 0.2, self.y, 30, 2)
        self.locked = false
    end)
end

function menu:FlyIn2(dir)
    self.alpha = 1
    if dir == 'left' then
        self.x = screen.width * 0.5 - screen.width
    elseif dir == 'right' then
...
function menu:FlyOut(dir)
    local x
    if dir == 'left' then
        x = screen.width * 0.2 - screen.width
    elseif dir == 'right' then
...
function simple_menu:render()
    SetViewMode('ui')
    ui.DrawMenu2(self.title, self.text, self.pos, self.x + self.offx, self.y, self.alpha, self.timer, self.pos_changed)
end


music_simple_menu = Class(object)

function music_simple_menu:init(title, content, keyslot, offx)
    self.layer = LAYER_TOP
    self.group = GROUP_GHOST
    self.alpha = 1
    self.offx = offx or 0
...
function music_simple_menu:frame()
    task.Do(self)
    if self.locked then
        return
    end
...
function music_simple_menu:render()
    SetViewMode('ui')
    ui.DrawMenu3(self.title, self.text, self.pos, self.x + self.offx, self.y, self.alpha, self.timer, self.pos_changed)
end
